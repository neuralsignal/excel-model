...
YAML-driven Excel financial model generator. Build professional financial models (P&L, DCF, Budget vs Actuals, Scenario Analysis, Comparison) from declarative YAML specs. Generates `.xlsx` workbooks with named ranges, styled sheets, and Excel formulas using openpyxl.

## Repo Structure

| Path | Purpose |
|------|---------|
| `excel_model/` | Source package |
| `excel_model/cli.py` | CLI entry point (click) |
| `excel_model/config.py` | Config dataclasses + YAML loading |
| `excel_model/exceptions.py` | Custom exception classes |
| `excel_model/spec.py` | Spec dataclasses (ModelSpec, LineItem, Assumption, etc.) |
| `excel_model/spec_loader.py` | YAML spec parsing via strictyaml |
| `excel_model/spec_schema.py` | Strictyaml schema definitions |
| `excel_model/validator.py` | Spec + input validation |
| `excel_model/formula_engine.py` | Formula rendering (21 formula types) |
| `excel_model/time_engine.py` | Period generation (annual, quarterly, monthly) |
| `excel_model/excel_writer.py` | Workbook assembly orchestrator |
| `excel_model/named_ranges.py` | Named range generation |
| `excel_model/style.py` | Cell styling and formatting |
| `excel_model/loader.py` | Input data loading (CSV, Excel, Parquet via polars) |
| `excel_model/defaults/` | Bundled default config (default_style.yaml) |
| `excel_model/models/` | Sheet builders per model type |
| `excel_model/models/_sheet_builder.py` | Shared sheet builder utilities |
| `excel_model/models/p_and_l.py` | P&L model builder |
| `excel_model/models/dcf.py` | DCF model builder |
| `excel_model/models/budget_vs_actuals.py` | Budget vs Actuals model builder |
| `excel_model/models/scenario.py` | Scenario analysis model builder |
| `excel_model/models/comparison.py` | Cross-entity comparison model builder |
...
- Use custom exceptions (e.g., `ExcelModelError`, `SpecValidationError`, `FormulaError`, `InputDataError`, `StyleConfigError`).
...
- **Inward Dependency Direction** -- High-level orchestration (excel_writer) does not import from low-level implementation details. One model builder does not import from another.
...
- No circular dependencies. Packages (`excel_model/`) never import from scripts (`scripts/`) or tests.
...
```
spec YAML --> validate --> build --> Excel workbook (.xlsx)
```
